...
const destroyProperty = (publicId) => {
    if (!window.confirm(props.translations.alerts.delete_confirmation)) {
        return
    }

    router.delete(route('properties.destroy', publicId), {
        preserveScroll: true,
    })
}
</script>
            ...
            <thead>
                <tr class="border-b border-black/10 dark:border-white/10">
                    ...
                    <th class="px-6 py-4 text-start text-[11px] font-medium uppercase tracking-[0.2em] text-black/40 dark:text-white/40">
                        {{ translations.table.rental_type }}
                    </th>
                ...
                >
                    <td class="w-full px-6 py-5 align-middle">
                        ...
                        <div class="mt-1 text-xs text-black/45 dark:text-white/45">
                            ...
                            <template v-if="property.country">
                                · {{ property.country.flag_emoji }} {{ property.country.name }}
                            </template>
                        </div>
                    </td>

                    <td class="px-6 py-5 align-middle">
                        <span class="text-sm text-nowrap text-black/65 dark:text-white/65">
                            {{ property.rental_type_label }}
                        </span>
                    </td>

                    <td class="px-6 py-5 align-middle">
                        <div
                            v-if="property.address_line_1"
                    ...
                    <td class="px-6 py-5 align-middle text-end">
                        <div class="flex justify-end gap-2">
                            <Link
                                :href="route('properties.edit', property.public_id)"
                            ...
                            <button
                                ...
                                @click="destroyProperty(property.public_id)"
                            >
