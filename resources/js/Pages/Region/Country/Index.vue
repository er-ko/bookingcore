...
const allCurrentPageSelected = computed(() => {
    ...
    return items.value.every(country => selectedIds.value.includes(country.id))
})
...
function toggleSelectAllCurrentPage() {
    const pageIds = items.value.map(country => country.id)
...
function toggleSelection(countryId) {
    if (selectedIds.value.includes(countryId)) {
        selectedIds.value = selectedIds.value.filter(id => id !== countryId)
        return
    }

    selectedIds.value = [...selectedIds.value, countryId]
}
...
function updateStatus({ scope, ids = [], isActive }) {
    ...
    router.patch(route('region.countries.status'), {
        scope,
        country_ids: ids,
        is_active: isActive,
    }, {
...
function activateSelected() {
    ...
    updateStatus({ scope: 'selected', ids: selectedIds.value, isActive: true })
}

function deactivateSelected() {
    if (!selectedIds.value.length) return
    updateStatus({ scope: 'selected', ids: selectedIds.value, isActive: false })
}

function activateAll() {
    updateStatus({ scope: 'all', isActive: true })
}

function deactivateAll() {
    updateStatus({ scope: 'all', isActive: false })
}

function activateOne(countryId) {
    updateStatus({ scope: 'single', ids: [countryId], isActive: true })
}

function deactivateOne(countryId) {
    updateStatus({ scope: 'single', ids: [countryId], isActive: false })
}
</script>
...
<template>
    ...
    <AppLayout>
        <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
            ...
            <div
                v-if="items.length > 0"
                class="mt-8 mb-4 flex flex-col gap-3 rounded-3xl border border-black/10 px-4 py-4 dark:border-white/10 md:flex-row md:items-center md:justify-between md:gap-1.5"
            >
