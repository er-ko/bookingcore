...
import CurrencyTable from '@/Components/Region/Currency/CurrencyTable.vue'
...
const items = computed(() => props.currencies?.data ?? [])
const links = computed(() => props.currencies?.meta?.links ?? [])
const hasPagination = computed(() => (props.currencies?.meta?.last_page ?? 1) > 1)
...
const allCurrentPageSelected = computed(() => {
    ...
    return items.value.every(currency => selectedIds.value.includes(currency.id))
})
...
function toggleSelectAllCurrentPage() {
    const pageIds = items.value.map(currency => currency.id)
...
function toggleSelection(currencyId) {
    if (selectedIds.value.includes(currencyId)) {
        selectedIds.value = selectedIds.value.filter(id => id !== currencyId)
        return
    }

    selectedIds.value = [...selectedIds.value, currencyId]
}
...
function updateStatus({ scope, ids = [], isActive }) {
    ...
    router.patch(route('region.currencies.status'), {
        scope,
        currency_ids: ids,
        is_active: isActive,
    }, {
...
function activateOne(currencyId) {
    updateStatus({
        scope: 'single',
        ids: [currencyId],
        isActive: true,
...
function deactivateOne(currencyId) {
    updateStatus({
        scope: 'single',
        ids: [currencyId],
        isActive: false,
