...
final class CreateBookingRequest extends FormRequest
{
    ...
    public function toDTO(): CreateBookingData
    {
        ...
        $branch = Branch::where('public_id', $validated['branch_public_id'])->firstOrFail();
        $unit = Unit::where('public_id', $validated['unit_public_id'])->firstOrFail();
        $activity = Activity::where('public_id', $validated['activity_public_id'])->firstOrFail();
